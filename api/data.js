...
// Permite que variantes da ferramenta (ex: uma versão "TC") guardem seus
// dados separadamente no mesmo banco, via /api/data?ns=tc. Sem o parâmetro,
// usa a chave padrão de sempre — então nada muda pra quem já usa.
function resolveKey(req) {
  try {
    const url = new URL(req.url, 'http://localhost');
    const ns = url.searchParams.get('ns');
    if (ns && /^[a-zA-Z0-9_-]{1,32}$/.test(ns)) return `${KEY}:${ns}`;
  } catch (e) { /* usa a chave padrão */ }
  return KEY;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const r = await upstash(url, token, ['GET', resolveKey(req)]);
      if (!r.ok) {
    ...
    if (req.method === 'POST') {
      ...
      const r = await upstash(url, token, ['SET', resolveKey(req), serialized]);
